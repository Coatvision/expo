...
public class ContactsNextModule: Module {
  ...
  public func definition() -> ModuleDefinition {
    ...
    Class(ContactNext.self) {
      ...
      StaticAsyncFunction("getAll") { () in
        return try ContactNext.getAll(contactRepository: contactRepository, contactFactory: contactFactory)
      }
      
      StaticAsyncFunction("getCount") { () in
        return try ContactNext.getCount(contactRepository: contactRepository)
      }
      
      StaticAsyncFunction("hasAnyContacts") { () in
        return try ContactNext.hasAnyContacts(contactRepository: contactRepository)
      }
      ...
      StaticAsyncFunction("getAllDetails") { (fields: [ContactField]) in
        return try ContactNext.getAllDetails(fields: fields, contactRepository: contactRepository, imageService: imageService)
      }
    ...
    Class(Group.self) {
      ...
      AsyncFunction("getContacts") { (this: Group) in
        try this.getContacts()
      }
    ...
    Class(Container.self) {
      ...
      AsyncFunction("getContacts") { (this: Container) in
        try this.getContacts()
      }
