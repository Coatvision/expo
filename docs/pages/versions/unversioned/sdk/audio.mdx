...
<ConfigPluginProperties
  properties={[
    ...
    },
    {
      ...
      description:
        'A boolean that determines whether to enable background audio recording. On iOS, this adds the `audio` background mode. On Android, this adds foreground service permissions and displays a persistent notification during recording. **Note:** Background recording can significantly impact battery life.',
      default: 'false',
    },
  ]}
/>
...
To enable background recording, use the config plugin in your **app.json**:
...
This automatically configures the required native settings:
...
<Collapsible summary="Manual configuration for bare workflow">

If you're not using [CNG](/workflow/continuous-native-generation/), you'll need to manually configure the native projects:
...
On iOS, background recording continues seamlessly when the app backgrounds or the screen locks. No additional notification or indicator is shown to the user beyond the system status bar.
